...
case ${target} in
	android)
		export SYSROOT="${NDK_DIR}/platforms/android-21/arch-arm64"
		export MSD_CONFIGURE_OPTS="--host aarch64-linux-android --prefix=${MSD_DESTDIR}"
		export PATH=${PATH}:${NDK_DIR}/toolchains/aarch64-linux-android-4.9/prebuilt/${HOST}/bin/
		export CROSS_COMPILE=aarch64-linux-android
		export RANLIB=aarch64-linux-android-ranlib
		export CFLAGS="--sysroot=${SYSROOT} -nostdlib"
		export CPPFLAGS="-I${NDK_DIR}/platforms/android-21/arch-arm64/usr/include/"
		export LDFLAGS="--sysroot=${SYSROOT} -Wl,-rpath-link=${NDK_DIR}/platforms/android-21/arch-arm64/usr/lib/,-L${NDK_DIR}/platforms/android-21/arch-arm64/usr/lib/"
		export LIBS="-lc -lm"
...
then
	...
	cp ${BASE_DIR}/diag_helper/libs/arm64-v8a/libdiag-helper.so ${BASE_DIR}/prebuilt/
fi
